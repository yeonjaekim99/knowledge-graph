...
- 소유 작업: `STO-001`
- 규범 근거: ADR-005
- 적용 범위: Recall v1 production startup 이전 DB path·capability gate
...
않는다. WAL 설정, connection-local PRAGMA와 process 내부 writer 직렬화는 `STO-002`가
소유한다.
...
- `STO-002`: WAL 전환, connection factory, busy timeout과 단일 writer queue
...
capability probe는 위 작업을 대신하지 않고 DB에 영구 table이나 migration metadata를 남기지
않는다.
...
network filesystem, capability별 누락, driver 오류 redaction과 영구 schema 부재를 검증한다.
