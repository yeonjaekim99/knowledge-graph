...
> 현재 상태: **Phase 04와 Phase 06 병렬 진행 중 · REC-001/REC-002/RCL-001 완료, REC-003/REC-004/RCL-002/RCL-003 진행 중**
...
| 구현 준비 | 7 | 7 | 완료 |
| 제품 구현 | 27 | 66 | 진행 중 |
| 전체 | 34 | 73 | Phase 04·06 병렬 진행 중 |

진행률은 수동 퍼센트가 아니라 `DONE 작업 수 / 전체 작업 수`로만 표시한다. 상세 문서의
작업 상태와 증거가 원본이며 이 표는 각 PR에서 함께 갱신하는 roll-up이다.

첫 wave 중 REC-002는 [PR #36](https://github.com/yeonjaekim99/knowledge-graph/pull/36)의
구현·독립 review·전체 회귀와 상태 증거로 완료했다. 선행 조건이 해제된 REC-003과 기존
REC-004, RCL-002, RCL-003은 격리 branch에서 진행하며, 병합 시점마다 phase 상세 상태를
기준으로 이 표를 다시 계산한다.
...
| 04 | [`memory_record`](04-memory-record.md) | `IN_PROGRESS` | 2/8 | Phase 03 `DONE` | 안전한 기록 수직 경로 |
| 05 | [`memory_revise`](05-memory-revise.md) | `TODO` | 0/7 | Phase 04 `DONE` | 철회·교정·병합·별칭 |
| 06 | [`memory_recall`](06-memory-recall.md) | `IN_PROGRESS` | 1/9 | Phase 03 `DONE` | FTS·그래프·랭킹·응답 |
