...
| `E-RECALL-SURFACE` | [RCL-002 구현 결정](../implementation/rcl-002-query-surface.md), [domain test](../../test/unit/domain/rcl-002-query-surface.test.mjs), [application test](../../test/unit/application/rcl-002-query-surface.test.mjs), [SQLite test](../../test/integration/sqlite/rcl-002-query-surface.test.mjs) | PRJ-002 normalize를 재사용한 explicit/query-derived term, PRJ-003/007 terminal redirect 검증, scoped polysemy·first-occurrence dedupe, 50+1과 같은 RCL-001 snapshot의 typed read-only seam | PR/merge 증거는 현재 RCL-002 PR owner가 연결; FTS/surface-no-answer fallback은 RCL-003, BFS/path는 RCL-005, note/Answer와 public golden은 RCL-007/008 |
...
- [x] `RCL-002` | baseline: S01/S09/S21이 normalize·surface·alias seed 의미를 확인했다. | production: `E-RECALL-SURFACE`에서 query term 후보, canonical surface seed, scope·50+1·결정적 dedupe와 typed snapshot seam을 구현했다. PR review/merge 증거 전까지 roadmap 상태는 `IN_PROGRESS`다.
