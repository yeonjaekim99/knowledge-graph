...
- active 제품 작업 완료 수는 현재 20/66이다. `FND-001`~`FND-005`, `FND-007`, `STO-001`~`008`과
  `PRJ-001`~`006`이 production artifact와 검증·PR 증거를 갖춰 `DONE`이며 나머지 active 작업은 각
...
- Phase 01은 6/6, Phase 02는 8/8로 종료됐고 Phase 03은 6/10이다. 다음 dependency-ready
  작업은 merge·alias rewrite인 `PRJ-007`과 TTL·aggregate인 `PRJ-008`이다.
