...
| `E-RUNTIME` | [PR #8](https://github.com/yeonjaekim99/knowledge-graph/pull/8), [결정적 runtime 경계](../implementation/fnd-003-runtime-boundaries.md) | request-scoped runtime port, Node clock/CSPRNG·canonical ULID/token, 고정 evaluation time, deterministic provider와 domain ambient-runtime guard | concrete scope/metadata adapter는 별도 evidence, journal 연결·충돌 retry와 public schema는 후속 작업 |
...
- [x] `PRJ-001` | baseline: S02/S23이 반복 replay와 canonical checksum 결정성을 확인했고 `E-RUNTIME`이 rules/clock port를 제공한다. | production: projection meta·scope replay 계약과 canonical dump를 구현하고 runtime 입력을 연결한다.
...
- active 제품 작업 완료 수는 현재 12/66이다. `FND-001`~`FND-005`, `FND-007`과 `STO-001`~`006`이
...
- Phase 01은 6/6으로 종료됐고 Phase 02는 6/8이다. trusted scope/metadata seam이 완료되어
  append-only journal의 `STO-007`이 다음 planning에서 시작할 수 있다.
