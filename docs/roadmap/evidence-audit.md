...
- [x] `RCL-005` | baseline: S12/S13이 양방향 BFS·literal 수집·최단 path·fanout 신호를 확인했다. | production: `E-RECALL-TRAVERSAL`에서 TEMP valid graph source와 bounded traversal/collection, reached parent/path, descriptor snapshot·fresh error, scope·snapshot·read-only fixture를 로컬 구현했다. 독립 재review·PR merge 증거는 아직 남아 있다.
