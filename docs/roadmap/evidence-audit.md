...
| `E-RECORD-CONTRACT` | [record 계약 결정](../implementation/rec-001-record-contract.md), [runtime contract test](../../test/contract/mcp/rec-001-record-schema.test.mjs), [type fixture](../../test/unit/application/rec-001-record-contract.type-test.ts) | frozen Record/ClaimDraft/Result source와 추론 DTO, trim 후 Unicode bounds, closed XOR·재해석 pair, schema/rejected 경계, result index coverage와 accepted status precedence | 비밀 탐지·마스킹, entity 해석·dedupe, journal/project application path와 실제 MCP wiring은 REC-002~008/MCP owner; PR review와 main merge 전에는 REC-001 상태를 DONE으로 올리지 않음 |
...
- [x] `REC-001` | baseline: ADR-013과 S03/S11/S14/S16/S18이 record 입력·결과 의미를 확인했고 `E-SCHEMA`가 규범 source·type/runtime seam을 제공한다. | production: `E-RECORD-CONTRACT`에서 실제 Record JSON Schema, 추론 DTO와 domain 결과 불변식을 구현했으며 PR review/main 병합 뒤 작업 상태를 완료한다.
